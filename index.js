...
`,fe="/JS-project/assets/icons-DZ36DW-g.svg";let k=null,q=null;async function j(){try{if(!document.getElementById("svg-sprite")){const o=await(await fetch(fe)).text(),r=document.createElement("div");r.id="svg-sprite",r.style.position="absolute",r.style.width="0",r.style.height="0",r.style.overflow="hidden",r.innerHTML=o,document.body.prepend(r)}document.getElementById("order-backdrop")||document.body.insertAdjacentHTML("beforeend",me);const e=document.getElementById("order-backdrop"),t=e?.querySelector(".modal-button-close"),s=e?.querySelector(".modal-form");e?.addEventListener("click",n=>{n.target===e&&u()}),document.addEventListener("keydown",n=>{n.key==="Escape"&&e?.classList.contains("is-open")&&u()}),t?.addEventListener("click",u),s?.addEventListener("submit",ge),pe(e)}catch(e){console.error("Помилка завантаження модалки:",e)}}function u(){const e=document.getElementById("order-backdrop");e&&(e.classList.remove("is-open"),document.body.classList.remove("no-scroll"),e.querySelector(".modal-form")?.reset(),k=null,q=null)}function ve(e){if(!e.name||e.name.length<2)throw new Error("Імʼя має містити щонайменше 2 символи");if(!/^\+?\d{10,15}$/.test(e.phone))throw new Error("Введіть коректний номер телефону")}async function he(e,t){return new Promise((s,n)=>{setTimeout(()=>{Math.random()<.85?s({ok:!0,json:async()=>({message:"Замовлення успішно прийнято"})}):n(new Error("Сервер тимчасово недоступний"))},800)})}async function ge(e){e.preventDefault();const t=e.target,s={name:t.elements["user-name"]?.value?.trim()||"",phone:t.elements["user-phone"]?.value?.trim()||"",comment:t.elements["user-comment"]?.value?.trim()||"",furnitureId:k,marker:q};try{ve(s);const n=await he("/orders",s);if(!n.ok){const o=await n.json().catch(()=>({}));throw new Error(o?.message||"Помилка запиту")}f.success({title:"Успіх",message:"Форма успішно відправлена",position:"topRight"}),u()}catch(n){f.error({title:"Помилка",message:n?.message||"Щось пішло не так",position:"topRight"})}}function C(e=!0,t=null,s=null){const n=document.getElementById("order-backdrop");n&&(n.classList.toggle("is-open",e),document.body.classList.toggle("no-scroll",e),e&&(k=t,q=s,n.querySelector("#user-name")?.focus()))}function pe(e){if(!e)return;e.querySelectorAll("button").forEach(s=>{s.style.cursor="pointer",s.addEventListener("mouseenter",()=>s.classList.add("hover")),s.addEventListener("mouseleave",()=>s.classList.remove("hover")),s.addEventListener("focus",()=>s.classList.add("focus")),s.addEventListener("blur",()=>s.classList.remove("focus")),s.addEventListener("mousedown",()=>s.classList.add("active")),s.addEventListener("mouseup",()=>s.classList.remove("active")),s.addEventListener("mouseout",()=>s.classList.remove("active"))})}j();const h=document.querySelector(".modal"),H=document.querySelector(".modal_close"),x=document.querySelector(".modal_main-image img"),N=document.querySelector(".modal_title"),ye=document.querySelector(".modal_category"),be=document.querySelector(".modal_price"),we=document.querySelector(".modal_description"),w=document.querySelector(".modal_rate"),L=document.querySelector(".color-checkboxes"),Le=document.querySelector(".modal_sizes"),a=document.querySelector(".modal_order-btn"),R=document.querySelector(".modal_thumbnails"),Ee=document.querySelector(".furniture-list");let g=0,m=[];function z(e){e.key==="Escape"&&b()}function W(){if(!m.length)return;x.src=m[g],x.alt=N.textContent||"Фото товару";const e=m.map((t,s)=>({src:t,i:s})).filter(({i:t})=>t!==g).map(({src:t,i:s})=>`
    ...
    `}else w.innerHTML="";L&&Array.isArray(e.color)?L.innerHTML=e.color.map((t,s)=>{const n=`color-${s}`;return`
        ...
        `).join("");qe.insertAdjacentHTML("beforeend",s),Ce(),p=new G(".reviews-swiper",{modules:[K,Q,Z],infinite:!1,slidesPerView:3,direction:"horizontal",loop:!1,pagination:{el:".reviews-actual-slide",clickable:!0,renderBullet:function(n,o){const r=this.activeIndex||0,c=n===r?"icon-active":"";return`
